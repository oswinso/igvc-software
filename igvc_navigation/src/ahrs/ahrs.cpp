...
using AHRS_StateVector = UKF::StateVector<
    UKF::Field<Position, UKF::Vector<2>>,
    UKF::Field<Velocity, UKF::Vector<1>>,
    UKF::Field<Acceleration, UKF::Vector<1>>,
...
using AHRS_SensorErrorVector = UKF::StateVector<
    UKF::Field<AccelerometerBias, UKF::Vector<3>>, UKF::Field<GyroscopeBias, UKF::Vector<3>>,
    UKF::Field<MagnetometerBias, UKF::Vector<3>>, UKF::Field<MagnetometerScaleFactor, UKF::Vector<3>>,
    UKF::Field<MagneticFieldNorm, real_t>, UKF::Field<MagneticFieldInclination, real_t>
>;
...
using AHRS_MeasurementVector =
    UKF::DynamicMeasurementVector<UKF::Field<Accelerometer, UKF::Vector<3>>, UKF::Field<Gyroscope, UKF::Vector<3>>,
...
template <>
template <>
UKF::Vector<3> AHRS_MeasurementVector::expected_measurement<AHRS_StateVector, Accelerometer, AHRS_SensorErrorVector>(
...
template <>
template <>
UKF::Vector<3> AHRS_MeasurementVector::expected_measurement<AHRS_StateVector, Gyroscope, AHRS_SensorErrorVector>(
    const AHRS_StateVector &state, const AHRS_SensorErrorVector &input)
{
...
template <>
template <>
UKF::Vector<3> AHRS_MeasurementVector::expected_measurement<AHRS_StateVector, Magnetometer, AHRS_SensorErrorVector>(
    const AHRS_StateVector &state, const AHRS_SensorErrorVector &input)
{
  return input.get_field<MagnetometerBias>().array() +
         input.get_field<MagnetometerScaleFactor>().array() *
             (state.get_field<Attitude>() *
              UKF::Vector<3>(input.get_field<MagneticFieldNorm>() *
...
using AHRS_Filter = UKF::SquareRootCore<AHRS_StateVector, AHRS_MeasurementVector, UKF::IntegratorHeun>;
...
template <>
template <>
UKF::Vector<3> AHRS_MeasurementVector::expected_measurement<AHRS_SensorErrorVector, Accelerometer, AHRS_StateVector>(
...
template <>
template <>
UKF::Vector<3> AHRS_MeasurementVector::expected_measurement<AHRS_SensorErrorVector, Gyroscope, AHRS_StateVector>(
    const AHRS_SensorErrorVector &state, const AHRS_StateVector &input)
{
...
template <>
template <>
UKF::Vector<3> AHRS_MeasurementVector::expected_measurement<AHRS_SensorErrorVector, Magnetometer, AHRS_StateVector>(
    const AHRS_SensorErrorVector &state, const AHRS_StateVector &input)
{
  return state.get_field<MagnetometerBias>().array() +
         state.get_field<MagnetometerScaleFactor>().array() *
             (input.get_field<Attitude>() *
              UKF::Vector<3>(state.get_field<MagneticFieldNorm>() *
...
using AHRS_ParameterEstimationFilter =
    UKF::SquareRootParameterEstimationCore<AHRS_SensorErrorVector, AHRS_MeasurementVector>;
...
AHRS_MeasurementVector::CovarianceVector AHRS_MeasurementVector::measurement_root_covariance(
    (AHRS_MeasurementVector::CovarianceVector() << 0.5 * UKF::Vector<3>::Ones(), 0.004 * UKF::Vector<3>::Ones(),
...
void ukf_init()
{
  ...
  ahrs.root_covariance.diagonal() << 1e0, 1e0, 3.2e0, 1e-3 * UKF::Vector<3>::Ones();

  /* Set process noise covariance. */
  ahrs.process_noise_root_covariance = AHRS_StateVector::CovarianceMatrix::Zero();
  ahrs.process_noise_root_covariance.diagonal() << 5e-5 * UKF::Vector<3>::Ones(), 5e-3 * UKF::Vector<3>::Ones();
  ...
  ahrs_errors.root_covariance.diagonal() << 0.8 * UKF::Vector<3>::Ones(), 0.02 * UKF::Vector<3>::Ones(),
      5.0e-2 * UKF::Vector<3>::Ones(), 1.0e-1 * UKF::Vector<3>::Ones(), 0.4, 0.7;
  ...
  */
   ...
   * UKF::Vector<3>::Ones(),
   7f * UKF::Vector<3>::Ones(),
      5f * UKF::Vector<3>::Ones(), 1e-6f * UKF::Vector<3>::Ones(),
    1e-7f;
}
...
void ukf_set_attitude(real_t w, real_t x, real_t y, real_t z)
{
  ...
  s.state.set_field<Attitude>(UKF::Quaternion(w, x, y, z));
}
...
void ukf_set_angular_velocity(real_t x, real_t y, real_t z)
{
  ...
  s.state.set_field<AngularVelocity>(UKF::Vector<3>(x, y, z));
}

void ukf_get_state(struct ukf_state_t *i)
{


  >attitude[0] = ahrs.state.get_field<Attitude>().x();
...
void ukf_set_state(struct ukf_state_t *i)
{


  s.state.set_field<Attitude>(
  uaternion(in->attitude[3], in->attitude[0], in->attitude[1], in->attitude[2]));
...
void ukf_get_state_covariance(
    real_t state_covariance[AHRS_StateVector::covariance_size() * AH R S_StateVector::covariance_size()

]) {

E
  en::Map<typename AHRS_StateVector::CovarianceMatrix> covariance_map(state_covariance);
co
  ariance_map = ahrs.root_covariance * ahrs.root_covariance.transpose();
}

void ukf_get_state_covariance_diagonal(
   state_covariance_diagonal[AHRS_StateVector::covariance_size()])
{


  en::Map<UKF::Vector<AHRS_StateVector::covariance_size()>> covariance_map(state_covariance_diagonal);
  covariance_map = (ahrs.root_covariance * ahrs.root_covariance.transpose()).diagonal();
}

void ukf_get_state_error(struct ukf_state_error_t *i)
{


  S_StateVector::StateVectorDelta state_error;
...
*/
void ukf_get_innovation(struct ukf_innovation_t *i)
{


  >accel[0] = ahrs.innovation[0];
...
void ukf_sensor_set_accelerometer(real_t x, real_t y, real_t z)
{
  ...
  s.set_field<Accelerometer>(UKF::Vector<3>(x, y, z));
}
...
void ukf_sensor_set_gyroscope(real_t x, real_t y, real_t z)
{
  ...
  s.set_field<Gyroscope>(UKF::Vector<3>(x, y, z));
}
...
void ukf_sensor_set_magnetometer(real_t x, real_t y, real_t z)
{
  ...
  s.set_field<Magnetometer>(UKF::Vector<3>(x, y, z));
}

void ukf_set_params(struct ukf_sensor_params_t *i)
{


  S_MeasurementVector::measurement_root_covariance <<
   qrt(in->accel_covariance[0]), st
         qrt(in->accel_covariance[1]), std::sqrt(in->accel_covariance[2]),
   qrt(in->gyro_covariance[0]), st
         qrt(in->gyro_covariance[1]), std::sqrt(in->gyro_covariance[2]),
   qrt(in->mag_covariance[0]), st
         qrt(in->mag_covariance[1]), std::sqrt(in->mag_covariance[2]);
}
...
void ukf_iterate(float dt)
{
  ...
  switch (st ep++) {
    ...
    e 0:
      ...
      */
      ...
      s_errors.innovation_step(meas, ahrs.state);
    ...
    case 1:
      ...
      s_errors.state.set_field<MagneticFieldNorm>(

          ::max(0.2f, std::min(0.7f, ahrs_errors.state.get_field<MagneticFieldNorm>())));



      ::Vector<3> temp;

      p = ahrs_errors.state.get_field<AccelerometerBias>();

      p[0] = std::max(real_t(-G_ACCEL / 4.
   ,std::min(real_t(G_ACCEL / 4.
   ,temp[0]));

      p[1] = std::max(real_t(-G_ACCEL / 4.
   ,std::min(real_t(G_ACCEL / 4.
   ,temp[1]));

      p[2] = std::max(real_t(-G_ACCEL / 4.
   ,std::min(real_t(G_ACCEL / 4.
   ,temp[2]));

      s_errors.state.set_field<AccelerometerBias>(temp);



      p = ahrs_errors.state.get_field<MagnetometerScaleFactor>();

      p[0] = std::max(real_t(0.5f), std::min(2.0f, temp[0]));

      p[1] = std::max(real_t(0.5f), std::min(2.0f, temp[1]));

      p[2] = std::max(real_t(0.5f), std::min(2.0f, temp[2]));

      s_errors.state.set_field<MagnetometerScaleFactor>(temp);
  ...
  */
  ...
  acceleration << meas.get_field<Accelerometer>() - ahrs_errors.state.get_field<AccelerometerBias>() -
                  ...
                  rs.state.get_field<Attitude>() * UKF::Vector<3>(0, 0, -G_ACCEL));
}
...
void ukf_set_process_noise(real_t process_noise_covariance[AHRS_StateVector::covariance_size()])
{
  ...
  en::Map<typename AHRS_StateVector::StateVectorDelta> covariance_map(process_noise_covariance);
  ahrs.process_noise_root_covariance = AHRS_StateVector::CovarianceMatrix::Zero();
...
void ukf_get_parameters(struct ukf_sensor_errors_t *i)
{


  >accel_bias[0] = ahrs_errors.state.get_field<AccelerometerBias>()[0];
...
void ukf_get_parameters_error(struct ukf_sensor_errors_t *i)
{


  S_SensorErrorVector::StateVectorDelta parameters_error;
  parameters_error =
...
uint32_t ukf_config_get_state_dim()
{
  ...
  urn AHRS_StateVector::covariance_size();
}
...
uint32_t ukf_config_get_measurement_dim()
{
  ...
  urn AHRS_MeasurementVector::max_size();
}
...
enum ukf_precision_t ukf_config_get_precision()
{
  ...
  (si zeof(real_t) == 8) {
    ...
    urn UKF_PRECISION_DOUBLE;
  }
  el



    urn UKF_PRECISION_FLOAT;
  }
}
